...
async def routine_page(request: Request,
                       routine_type: str,
    ...
    today_session = db.query(models.WorkoutSession).filter(
        ...
        models.WorkoutSession.date >= today_start, models.WorkoutSession.date
        <= today_end).first()
...
async def save_session(session_data: Dict[str, Any],
    ...
    try:
        ...
        existing_session = db.query(models.WorkoutSession).filter(
            ...
            models.WorkoutSession.date <= today_end).first()

        if existing_session:
            # 기존 세션이 있으면 관련된 모든 운동 세트 삭제
            db.query(models.ExerciseSet).filter(
                models.ExerciseSet.session_id == existing_session.id).delete()
            ...
            new_session = models.WorkoutSession(
                date=datetime.now(), routine_type=session_data["routine_type"])
            db.add(new_session)
            db.flush()  # ID 생성
            ...
            for set_idx, set_data in enumerate(exercise["sets"]):
                if set_data["status"] in ["success", "fail"]:  # 완료된 세트만 저장
                    exercise_set = models.ExerciseSet(
                        ...
                        set_number=int(set_data["set_number"]) if isinstance(
                            set_data["set_number"],
                            str) else set_data["set_number"],
...
async def update_weight_during_workout(routine_type: str = Form(...),
...
async def history_page(request: Request, db: Session = Depends(get_db)):
    ...
    sessions = db.query(models.WorkoutSession).order_by(
        models.WorkoutSession.date.desc()).all()
    ...
    for session in sessions:
        ...
        if session.routine_type not in routine_by_date[date_str]:
            ...
            sessions_by_date[date_str].append({
                ...
                session.id,
                "routine_type":
                session.routine_type,
                "date":
                session.date.strftime("%Y-%m-%dT%H:%M:%S")
            })
...
async def session_detail(request: Request,
    ...
    session = db.query(models.WorkoutSession).filter(
        models.WorkoutSession.id == session_id).first()
    if not session:
        raise HTTPException(status_code=404, detail="세션을 찾을 수 없습니다")

    # 해당 세션의 모든 운동 세트 가져오기
    exercise_sets = db.query(models.ExerciseSet).filter(
        models.ExerciseSet.session_id == session_id).all()
...
async def exercise_progress(exercise_name: str, db: Session = Depends(get_db)):
    ...
    exercise_sets = db.query(models.ExerciseSet, models.WorkoutSession).join(
        ...
        models.ExerciseSet.session_id == models.WorkoutSession.id).filter(
            models.ExerciseSet.exercise_name == exercise_name).order_by(
                models.WorkoutSession.date).all()
    ...
    for exercise_set, session in exercise_sets:
        ...
        if date_str not in progress_data:
            progress_data[date_str] = {"total_weight": 0, "count": 0}
    ...
    for date_str, data in progress_data.items():
        avg_weight = data["total_weight"] / data["count"] if data[
            "count"] > 0 else 0
        result.append({
            "date": date_str,
...
async def update_all_weights(routine_type: str = Form(...),
                             weight: float = Form(...)):
    settings = load_settings()
...
async def progress_graph_page(request: Request):
    ...
    return templates.TemplateResponse("progress_graph.html", {
        "request": request,
...
# 주간 및 월간 운동 횟수 API
@app.get("/api/workout-counts")
async def get_workout_counts(db: Session = Depends(get_db)):
    try:
        ...
        # 주간 운동 횟수
        week_count = db.query(models.WorkoutSession).filter(
            models.WorkoutSession.date >= week_start).count()

        # 월간 운동 횟수
        month_count = db.query(models.WorkoutSession).filter(
            models.WorkoutSession.date >= month_start).count()

        return {"week_count": week_count, "month_count": month_count}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


# 최근 운동 기록 API
@app.get("/api/recent-workouts")
async def get_recent_workouts(limit: int = 5, db: Session = Depends(get_db)):
    try:
        # 가장 최근 세션 가져오기
        recent_sessions = db.query(models.WorkoutSession).order_by(
            models.WorkoutSession.date.desc()).limit(limit).all()

        result = []
        for session in recent_sessions:
            # 해당 세션에 포함된 운동 종목 수 계산
            exercise_count = db.query(models.ExerciseSet.exercise_name).filter(
                models.ExerciseSet.session_id ==
            ...
            if session.date >= today:
                formatted_date = "오늘"
            elif session.date >= yesterday:
                formatted_date = "어제"
            else:
                # 일주일 이내면 요일, 그 외에는 날짜 표시
                days_diff = (today.date() - session.date.date()).days
                if days_diff < 7:
                    weekday_names = [
                        "월요일", "화요일", "수요일", "목요일", "금요일", "토요일", "일요일"
                    ]
                    weekday = session.date.weekday()
                    formatted_date = weekday_names[weekday]
                else:
                    formatted_date = session.date.strftime("%m월 %d일")
